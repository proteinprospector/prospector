				...
				if ( spectrumRange.startReadingFile ( fraction, count ) ) {
					...
					if ( !binaryDataArray ) {
						...
						if ( !strcmp ( name, "referenceableParamGroupRef" ) ) {
							string ref;
							getAttributeValue ( attributes, "ref", ref );
							if ( ref == "mz_params" )				mzArrayFlag = true;
							else if ( ref == "int_params" )			intenArrayFlag = true;
							else if ( ref == "charge_params" )		chargeArrayFlag = true;
							std::map <std::string, MapStringToString>::const_iterator cur = paramGroups.find ( ref );
							if ( cur != paramGroups.end () ) {
								if ( (*cur).second.find ( "64-bit float" ) != (*cur).second.end () ) 	precision = 64;
								if ( (*cur).second.find ( "32-bit float" ) != (*cur).second.end () )	precision = 32;
								if ( (*cur).second.find ( "no compression" ) != (*cur).second.end () )	compression = false;
								if ( (*cur).second.find ( "zlib compression" ) != (*cur).second.end () )compression = true;
							}
						}
...
void PPExpatMZMLData::endElement ( const char* name )
{
	if ( binaryFlag ) {
		...
		if ( !strcmp ( name, "binaryDataArray" ) ) {
			...
			else if ( chargeArrayFlag ) {
				if ( compression ) {
					if ( precision == 32 ) makeVector32Uncompress ( s, 0, false, chargeList );
					else makeVector64Uncompress ( s, 0, false, chargeList );
				}
				else {
					if ( precision == 32 ) makeVector32 ( s, false, chargeList );
					else makeVector64 ( s, false, chargeList );
				}
			}
			s = "";
			binaryDataArray = false;
	...
	for ( DoubleVectorSizeType i = 0 ; i < mzList.size () ; i++ ) {
		if ( i < chargeList.size () ) {
			if ( i < intensityList.size () ) {
				msmsDataPoint.addPeak ( mzList [i], (int) ( chargeList [i] + 0.5 ), intensityList [i] );
			}
			else {
				msmsDataPoint.addPeak ( mzList [i], (int) ( chargeList [i] + 0.5 ), 0.0 );
			}
		}
		else {
			if ( i < intensityList.size () ) {
...
PPExpatPepXML::PPExpatPepXML ( const string& fractionName, StringVectorVector& headerLines, StringVectorVector& rows, int& zColumnNumber, int& fractionColumnNumber, int& scanIDColumnNumber, int& peptideColumnNumber, int& variableModsColumnNumber ) :
	fractionName ( fractionName ),
	headerLines ( headerLines ),
	rows ( rows ),
	zColumnNumber ( zColumnNumber ),
	fractionColumnNumber ( fractionColumnNumber ),
...
{
	if ( !strcmp ( name, "search_summary" ) ) {
		string searchEngine;;
		if ( getAttributeValue ( attributes, "search_engine", searchEngine ) ) {
			if ( isNoCasePrefix ( searchEngine, "Spectrum Mill" ) ) {
				indexFlag = true;
			}
		}
	}
	if ( !strcmp ( name, "spectrum_query" ) ) {
		...
		precursorMOverZFlag = getAttributeValue ( attributes, "precursor_m_over_z", mOverZ );	// Spectrum Mill has this attribute
		return;
	}
	if ( !strcmp ( name, "search_hit" ) ) {
		getAttributeValue ( attributes, "peptide", dbPeptide );
		int parentCharge = 0;
		if ( getAttributeValue ( attributes, "parentCharge", parentCharge ) ) {
			z = parentCharge;
		}
		mod = "";
		return;
	...
	if ( !strcmp ( name, "search_score" ) ) {
		...
		MapStringToIntConstIterator cur = scoreHeadersMap.find ( scHeader );
		if ( cur == scoreHeadersMap.end () ) {
			scoreHeadersMap [scHeader] = scoreHeaders.size ();
			scoreHeaders.push_back ( scHeader );
			scores.push_back ( sc );
		}
		else
			scores [(*cur).second] = sc;
	}
}
void PPExpatPepXML::endElement ( const char* name )
{
	if ( !strcmp ( name, "search_hit" ) ) {
		...
		if ( precursorMOverZFlag )	cols.push_back ( gen_ftoa ( mOverZ, "%.4f" ) );
		else						cols.push_back ( gen_ftoa ( mToMOverZ ( precursorM, z, true ), "%.4f" ) );

		cols.push_back ( gen_itoa ( z ) );
		zColumnNumber = cols.size ();

		cols.push_back ( fractionName );
		fractionColumnNumber = cols.size ();

		cols.push_back ( gen_itoa ( index ) );
		if ( indexFlag ) scanIDColumnNumber = cols.size ();
		...
		for ( int i = 0 ; i < scores.size () ; i++ ) {
			cols.push_back ( scores [i] );
		}
		fill ( scores.begin (), scores.end (), "" );
		if ( scoreHeaders.size () > scoreHeadersSize ) {	// New score types discovered then redo the headers
			headerLines.clear ();
			StringVector header;
			header.push_back ( "m/z" );
			header.push_back ( "z" );
			header.push_back ( "Fraction" );
			...
			scoreHeadersSize = scoreHeaders.size ();
			for ( int i = 0 ; i < rows.size () ; i++ ) {
				rows [i].resize ( cols.size () );
			}
		}
		rows.push_back ( cols );
	}
